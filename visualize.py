...
data_config = utils.read_data('config.json')[opt.task]
opt.samples_datapath = opt.datapath+opt.task+"/samples" 
opt.datapath = opt.datapath+opt.task+"/"
opt.save_dir = opt.save_dir+opt.task+"/"
...
vcond = Variable(cond.cpu())
vtarget = Variable(target.cpu())
...
for mfile in glob.glob(opt.datapath+ '/baseline_model/model/*.model'):
    print('loading {}'.format(mfile))
    model = load_model(mfile)

    # make folder to save visualizations
    save_dir = opt.save_dir+mfile + '.viz'
...
for mfile in glob.glob(opt.datapath+ '/Sto*/model/*.model'):
    print('loading {}'.format(mfile))
    model = load_model(mfile)
    ...
    for k in range(n_batches):
        ...
        pred, g_pred, z = model(Variable(cond_.cpu()), Variable(target_.cpu()))
    ...
    #print(pred.size(),g_pred.size(),z_true.size(),vcond.size(),vtarget.size())
    g_pred = g_pred.view(vtarget.size())
    err = vtarget - g_pred
    pred = pred.data.view(opt.batch_size, opt.npred, opt.nc, opt.height, opt.width)
