...
class ImageLoader(object):
    ...
    def _load_set(self, split, n_episodes):
        datalist = []
        datapath = '{}/{}'.format(self.arg.samples_datapath, split)
        print(datapath)
...
#                print('loading {}'.format(abs_fdname))
    ...
    def __init__(self, arg):
        ...
        self.w = arg.width
        self.nc = arg.nc
        self.ncond = arg.ncond
        self.npred = arg.npred
        self.datalist_train = self._load_set('train', 50)
        self.datalist_valid = self._load_set('valid', 20)
        self.datalist_test = self._load_set('test', 20)
    ...
    def get_batch(self, split):
        ...
        # return tensor
        cond_frames_ts =tf.convert_to_tensor(cond_frames,tf.float32) 
        pred_frames_ts =tf.convert_to_tensor(pred_frames,tf.float32) 
        actions_ts =tf.convert_to_tensor(actions,tf.float32) 
        #print(cond_frames_ts)
    ...
    def get_iterated_batch(self, split):
        ...
        # rolling
        id = 1
        while id <= self.arg.get("batch_size"):
        ...
        #print(cond_frames_ts)
        return cond_frames_ts.cpu(), pred_frames_ts.cpu(), actions_ts.cpu()
